...
var RectangleTesterantoBaseTestImplementation = {
  ...
  thens: {
    AreaPlusCircumference: (combined) => (rectangle) => {
      assert2.equal(rectangle.area() + rectangle.circumference(), combined);
    },
    getWidth: (width) => (rectangle) => assert2.equal(rectangle.width, width),
    getHeight: (height) => (rectangle) => assert2.equal(rectangle.height, height),
    area: (area) => (rectangle) => assert2.equal(rectangle.area(), area),
    prototype: (name) => (rectangle) => assert2.equal(1, 1),
    circumference: (circumference) => (rectangle) => assert2.equal(rectangle.circumference(), circumference)
  },
  checks: {
    /* @ts-ignore:next-line */
    AnEmptyState: () => {
