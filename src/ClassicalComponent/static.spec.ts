...
import { I } from "testeranto/src/SubPackages/react-dom/component/node";
...
export const ClassicalComponentStaticSpec: ITestSpecification<I, O> = (
  Suite,
  Given,
  When,
  Then
...
export const ClassicalComponentStaticImpl: ITestImplementation<I, O> = {
  suites: {
  ...
  thens: {
    renderToStaticMarkup: (expectation) => async (reactNodes) => {
      assert.deepEqual(renderToStaticMarkup(reactNodes), expectation);
    },

    renderToStaticNodeStream: () => async (reactNodes) => {
